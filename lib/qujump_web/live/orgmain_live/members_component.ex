defmodule MembersComponent do
  ...
  def update(%{orgstruct_id: orgstruct_id} = assigns, socket) do    
    ...
    {employees, members } = if orgstruct,
    do: {
      Employees.list_employees(orgstruct_id: orgstruct_id),
      Employees.list_employee_members(orgstruct_id: orgstruct_id)},
    else: {[],[]}
  ...
  def handle_event("new", %{"id" => orgstruct_id} = _employee_params, socket) do
    ...
    {:noreply, socket
    |> assign(:member, %Employee{orgstruct_id: orgstruct.id})
    |> assign(:member_action, :new)
